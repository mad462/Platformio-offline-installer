...
export default class ProjectManager {
  ...
  async switchToProject(projectDir, options = {}) {
    ...
    const fallbackConfig = {
      ...
      get_default_env: () => 'default',
      getEnvPlatform: () => 'espressif32',
    };
    try {
      const cfg = await observer.getConfig();
      if (!cfg || typeof cfg.envs !== 'function') {
        console.warn('PlatformIO config is invalid. Using fallback config.');
        observer._config = fallbackConfig;
      }
    } catch (err) {
      console.error(err);
      console.warn('PlatformIO config failed. Using fallback config.');
      observer._config = fallbackConfig;
    }
    try {
  ...
  async pickProjectEnv() {
    ...
    for (const projectDir of projectHelpers.getPIOProjectDirs()) {
      ...
      try {
        const cfg = await observer.getConfig();
        envs = cfg && typeof cfg.envs === 'function' ? cfg.envs() : [];
      } catch (err) {
        console.error(err);
        continue;
      }
